...
void NasdaqClient::connect(std::string_view host, std::string_view port,
...
void NasdaqClient::loadStaticData(std::string_view stocksPath,
                                  std::string_view marketMakerPath) const {
   loadCSV("stocks", stocksPath);
   loadCSV("marketmakers", marketMakerPath);
}

void NasdaqClient::loadPremarketData(std::string_view ordersPath,
                                     std::string_view executionsPath,
                                     std::string_view cancellationsPath) const {
...
void NasdaqClient::sendOrder(const Order& order) const {
   ...
   paramValues[6] =
      order.attribution.empty() ? nullptr : order.attribution.c_str();
   ...
   if (PQsendQueryPrepared(conn, "newOrder", 8, paramValues, paramLengths,
                           nullptr, 0) != 1)
      throw std::runtime_error("Could not send new order");
}

void NasdaqClient::sendOrderbookAdd(uint64_t orderId, unsigned stockId,
   ...
   if (PQsendQueryPrepared(conn, "addToOrderbook", 5, paramValues, paramLengths,
                           nullptr, 0) != 1)
...
void NasdaqClient::sendOrderbookDelete(uint64_t orderId) const {
   ...
   if (PQsendQueryPrepared(conn, "deleteFromOrderbook", 1, paramValues,
                           paramLengths, nullptr, 0) != 1)
      throw std::runtime_error("could not delete from order book");
}

void NasdaqClient::sendOrderbookReduce(uint64_t orderId,
                                       unsigned quantity) const {
   ...
   if (PQsendQueryPrepared(conn, "reduceInOrderbook", 2, paramValues,
                           paramLengths, nullptr, 0) != 1)
...
void NasdaqClient::sendExecution(const Execution& execution) const {
   ...
   if (PQsendQueryPrepared(conn, "newExecution", 5, paramValues, paramLengths,
                           nullptr, 0) != 1)
...
void NasdaqClient::sendCancellation(const Cancellation& cancellation) const {
   ...
   if (PQsendQueryPrepared(conn, "newCancellation", 4, paramValues, paramLengths,
                           nullptr, 0) != 1)
...
void NasdaqClient::runExchange(const std::string& ordersPath,
   ...
   io::CSVReader<5, io::trim_chars<' '>, io::no_quote_escape<';'>>
      executionsReader(executionsPath);
   io::CSVReader<4, io::trim_chars<' '>, io::no_quote_escape<';'>>
      cancellationsReader(cancellationsPath);

   orderReader.read_header(io::ignore_extra_column, "stockId", "timestamp",
                           "orderId", "side", "quantity", "price", "attribution",
                           "prevOrder");
   executionsReader.read_header(io::ignore_extra_column, "timestamp", "orderId",
                                "stockId", "quantity", "price");
   cancellationsReader.read_header(io::ignore_extra_column, "timestamp",
                                   "orderId", "stockId", "quantity");

   prepare(conn, "newOrder",
           "INSERT INTO orders VALUES($1, $2, $3, $4, $5, $6, $7, $8);");
   prepare(conn, "newExecution",
           "INSERT INTO executions VALUES($1, $2, $3, $4, $5);");
   prepare(conn, "newCancellation",
           "INSERT INTO cancellations VALUES($1, $2, $3, $4);");
   prepare(conn, "addToOrderbook",
           "INSERT INTO orderbook VALUES($1, $2, $3, $4, $5);");
   prepare(conn, "deleteFromOrderbook",
           "DELETE FROM orderbook WHERE orderId = $1;");
   prepare(conn, "reduceInOrderbook",
           "UPDATE orderbook SET quantity = quantity - $2 WHERE orderId = $1;");
   ...
   orderReader.read_row(order.stockId, order.timestamp, order.orderId,
                        order.side, order.quantity, order.price,
                        order.attribution, order.prevOrder);
   executionsReader.read_row(execution.timestamp, execution.orderId,
                             execution.stockId, execution.quantity,
                             execution.price);
   cancellationsReader.read_row(cancellation.timestamp, cancellation.orderId,
                                cancellation.stockId, cancellation.quantity);
   uint64_t base = order.timestamp; // We treat the first order after all
   // preloaded orders as t0.

   while (true) {
      auto curTime = time_point_cast<std::chrono::nanoseconds>(
      ...
      // Start a transaction
      if (PQsendQueryPrepared(conn, "begin", 0, nullptr, nullptr, nullptr, 0) !=
          1)
      ...
      while (order.timestamp < limit) {
         ...
         sendOrderbookAdd(order.orderId, order.stockId, order.side, order.price,
                          order.quantity);
         ++counter;
         ...
         bool rowRead = orderReader.read_row(
            order.stockId, order.timestamp, order.orderId, order.side,
            order.quantity, order.price, order.attribution, order.prevOrder);
         if (!rowRead)
            break;
      ...
      while (execution.timestamp < limit) {
         ...
         bool rowRead = executionsReader.read_row(
            execution.timestamp, execution.orderId, execution.stockId,
      ...
      while (cancellation.timestamp < limit) {
         ...
         bool rowRead = cancellationsReader.read_row(
            cancellation.timestamp, cancellation.orderId, cancellation.stockId,
...
NasdaqClient::~NasdaqClient() { close(); }
...
void NasdaqClient::loadCSV(std::string_view tblName,
                           std::string_view path) const {
   assert(conn);
   std::stringstream stocksCmd;
   stocksCmd << "COPY " << tblName << " FROM '" << path
             << "' with(format text, delimiter ';', null '', header true);";
   exec(conn, stocksCmd.str());
}

void NasdaqClient::prepare(PGconn* conn, const std::string& name,
